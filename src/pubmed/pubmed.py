...
def fetch(query : str, max:int = 10, fileName : str=None):
    ...
    search_url = f"{base_url}/esearch.fcgi?db=pubmed&term={query.replace(' ', '+')}&retmode=json&retmax=2&usehistory=y"
    response = requests.get(search_url)
    (webenv, queryKey) = parse_json(response)
    fetch_url = f"{base_url}/efetch.fcgi?db=pubmed&query_key={queryKey}&WebEnv={webenv}&retmax={max}"
